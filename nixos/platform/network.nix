...
let
  ...
  interfaces = filter (i: i.vlan != "ipmi") (lib.attrValues fclib.network);
  ...
  udevRenameRules = pkgs.writeTextFile {
    ...
    text = if (interfaces != {}) then
        lib.concatMapStrings
          (interface: ''
            KERNEL=="eth*", ATTR{address}=="${interface.mac}", NAME="${interface.physical_device}"
            '') interfaces
      else ''
  ...
  config = rec {
    ...
    networking = {
      ...
      bridges = listToAttrs (map (interface:
        (lib.nameValuePair
            "${interface.device}"
            { interfaces = interface.attached_devices; }))
    ...
    systemd.services =
      ...
      (listToAttrs
        (map (interface:
          (lib.nameValuePair
            "network-link-properties-${interface.physical_device}"
            rec {
              description = "Ensure link properties for ${interface.physical_device}";
              ...
              script = ''
                IFACE=${interface.physical_device}
